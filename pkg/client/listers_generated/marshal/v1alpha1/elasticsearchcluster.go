...
// List lists all ElasticsearchClusters in the indexer.
func (s *elasticsearchClusterLister) List(selector labels.Selector) (ret []*v1alpha1.ElasticsearchCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ElasticsearchCluster))
	})
	return ret, err
}
...
// List lists all ElasticsearchClusters in the indexer for a given namespace.
func (s elasticsearchClusterNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ElasticsearchCluster, err error) {
...
// Get retrieves the ElasticsearchCluster from the indexer for a given namespace and name.
func (s elasticsearchClusterNamespaceLister) Get(name string) (*v1alpha1.ElasticsearchCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("elasticsearchcluster"), name)
	}
	return obj.(*v1alpha1.ElasticsearchCluster), nil
}
