...
// List lists all Pilots in the indexer.
func (s *pilotLister) List(selector labels.Selector) (ret []*v1alpha1.Pilot, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Pilot))
	})
	return ret, err
}
...
// List lists all Pilots in the indexer for a given namespace.
func (s pilotNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Pilot, err error) {
...
// Get retrieves the Pilot from the indexer for a given namespace and name.
func (s pilotNamespaceLister) Get(name string) (*v1alpha1.Pilot, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("pilot"), name)
	}
	return obj.(*v1alpha1.Pilot), nil
}
