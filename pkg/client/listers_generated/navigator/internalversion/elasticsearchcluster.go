...
type ElasticsearchClusterLister interface {
	...
	List(selector labels.Selector) (ret []*navigator.ElasticsearchCluster, err error)
...
// List lists all ElasticsearchClusters in the indexer.
func (s *elasticsearchClusterLister) List(selector labels.Selector) (ret []*navigator.ElasticsearchCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*navigator.ElasticsearchCluster))
	})
	return ret, err
}
...
type ElasticsearchClusterNamespaceLister interface {
	...
	Get(name string) (*navigator.ElasticsearchCluster, error)
...
// List lists all ElasticsearchClusters in the indexer for a given namespace.
func (s elasticsearchClusterNamespaceLister) List(selector labels.Selector) (ret []*navigator.ElasticsearchCluster, err error) {
...
// Get retrieves the ElasticsearchCluster from the indexer for a given namespace and name.
func (s elasticsearchClusterNamespaceLister) Get(name string) (*navigator.ElasticsearchCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(navigator.Resource("elasticsearchcluster"), name)
	}
	return obj.(*navigator.ElasticsearchCluster), nil
}
