...
type CassandraClusterLister interface {
	...
	List(selector labels.Selector) (ret []*navigator.CassandraCluster, err error)
...
// List lists all CassandraClusters in the indexer.
func (s *cassandraClusterLister) List(selector labels.Selector) (ret []*navigator.CassandraCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*navigator.CassandraCluster))
	})
	return ret, err
}
...
type CassandraClusterNamespaceLister interface {
	...
	Get(name string) (*navigator.CassandraCluster, error)
...
// List lists all CassandraClusters in the indexer for a given namespace.
func (s cassandraClusterNamespaceLister) List(selector labels.Selector) (ret []*navigator.CassandraCluster, err error) {
...
// Get retrieves the CassandraCluster from the indexer for a given namespace and name.
func (s cassandraClusterNamespaceLister) Get(name string) (*navigator.CassandraCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(navigator.Resource("cassandracluster"), name)
	}
	return obj.(*navigator.CassandraCluster), nil
}
